...
function show_admin_notice()
{
    ...
    if ( !class_exists( 'acf' ) || !class_exists( 'WPGraphQL' ) || defined( 'WPGRAPHQL_VERSION' ) && version_compare( WPGRAPHQL_VERSION, $wp_graphql_required_min_version, 'lt' ) ) {
        ...
        add_action( 'admin_notices', function () use( $wp_graphql_required_min_version ) {
            ?>
			<div class="error notice">
				<p><?php 
            _e( sprintf( 'Both WPGraphQL (v%s+) and Advanced Custom Fields (v5.7+) must be active for "wp-graphql-acf" to work', $wp_graphql_required_min_version ), 'wp-graphiql-acf' );
            ?></p>
			</div>
				<?php 
        } );
