<?php
if ( ! class_exists( 'ECACalendarFeedEventsManager' ) ) {
class ECACalendarFeedEventsManager extends ECACalendarFeed {
    ...
    function get_events( $start_date, $end_date, $data = array() ) {
	    ...
	    $event_results = EM_Events::get( apply_filters( 'eca_fetch_events_args-' . $this->get_identifier(), $filters, $start_date, $end_date, $data ) );

        foreach ( $event_results as $event ) {
	        $post = get_post( $event->post_id );
	        $location = $event->get_location();
            $image_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), apply_filters( 'eca_image_size', 'medium' ) );
            if ( !empty( $image_src ) )
            ...
            $retval[] = new ECACalendarEvent( array(
