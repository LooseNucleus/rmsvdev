<?php
if ( ! class_exists( 'ECACalendarFeedEventOrganiser' ) ) {
class ECACalendarFeedEventOrganiser extends ECACalendarFeed {
    ...
    function get_events( $start_date, $end_date, $data = array() ) {
	    ...
	    $events = eo_get_events( apply_filters( 'eca_fetch_events_args-' . $this->get_identifier(), array( 'posts_per_page' => 10000, 'post_status' => 'publish', 'event_start_after' => date( 'Y-m-d H:i', $start_date ), 'event_start_before' => date( 'Y-m-d H:i', $end_date ) ), $start_date, $end_date, $data ));
        ...
        foreach ( $events as $post ) {
	        ...
	        if ( strtotime( $current_start_date ) > $end_date )
		        break;
            $image_src = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), apply_filters( 'eca_image_size', 'medium', get_the_ID() ) );
            if ( !empty( $image_src ) )
                $image_url = $image_src[0];
            else
                $image_url = false;

            $retval[] = new ECACalendarEvent( apply_filters( 'eca_create_calendar_event_args-' . $this->get_identifier(), array(
