<?php
if ( ! class_exists( 'ECACalendarFeedAi1ec' ) ) {
class ECACalendarFeedAi1ec extends ECACalendarFeed {
    ...
    function get_events( $start_date, $end_date, $data = array() ) {
	    ...
	    $filters = apply_filters( 'eca_ai1ec_filters', $filters, $data );
        ...
        foreach ( $event_results as $event ) {
	        ...
	        if ( apply_filters( 'eca_ai1ec_recurring_once', false ) and in_array( $post->ID, $post_ids ) )
		        continue;
            ...
            $retval[] = new ECACalendarEvent( array(
                ...
                'event_cost' => ( $event->get( 'is_free' ) ? __( 'FREE', 'event-calendar-attendees' ) : $event->get( 'cost' ) ),
