<?php
if ( ! class_exists( 'ECACalendarEvent' ) ) {
class ECACalendarEvent {
	private $_plugin;
    ...
    public static function get_day_text( $number = 1 ) {
        return _n( 'day', 'days', $number, 'event-calendar-attendees' );
    }

    public static function get_week_text( $number = 1 ) {
        return _n( 'week', 'weeks', $number, 'event-calendar-attendees' );
    }

    public static function get_month_text( $number = 1 ) {
        return _n( 'month', 'months', $number, 'event-calendar-attendees' );
    }

    public static function get_year_text( $number = 1 ) {
        return _n( 'year', 'years', $number, 'event-calendar-attendees' );
    }

    public static function get_available_format_tags( $plugin_slug = false ) {
        $all_tags = array(
            'title' => __( 'Title', 'event-calendar-attendees' ),
	        'subtitle' => __( 'Subtitle', 'event-calendar-attendees' ),
            'description' => __( 'Description', 'event-calendar-attendees' ),
            'start_date' => __( 'Start Date', 'event-calendar-attendees' ),
            'start_time' => __( 'Start Time', 'event-calendar-attendees' ),
            'end_date' => __( 'End Date', 'event-calendar-attendees' ),
            'end_time' => __( 'End Time', 'event-calendar-attendees' ),
	        'instant_event' => __( 'No End Date Flag', 'event-calendar-attendees' ),
            'location_name' => __( 'Location Name', 'event-calendar-attendees' ),
            'location_address' => __( 'Location Address', 'event-calendar-attendees' ),
            'location_city' => __( 'Location City', 'event-calendar-attendees' ),
            'location_state' => __( 'Location State', 'event-calendar-attendees' ),
            'location_zip' => __( 'Location Zip/Postal', 'event-calendar-attendees' ),
            'location_country' => __( 'Location Country', 'event-calendar-attendees' ),
            'location_website' => __( 'Location Website', 'event-calendar-attendees' ),
	        'location_phone' => __( 'Location Phone', 'event-calendar-attendees' ),
            'contact_name' => __( 'Contact Name', 'event-calendar-attendees' ),
            'contact_info' => __( 'Contact Info', 'event-calendar-attendees' ),
            'contact_phone' => __( 'Contact Phone', 'event-calendar-attendees' ),
            'contact_website' => __( 'Contact Website', 'event-calendar-attendees' ),
            'contact_email' => __( 'Contact Email', 'event-calendar-attendees' ),
	        'organizer_name' => __( 'Organizer Name', 'event-calendar-attendees' ),
            'organizer_email' => __( 'Organizer Email', 'event-calendar-attendees' ),
            'organizer_website' => __( 'Organizer Website', 'event-calendar-attendees' ),
            'organizer_phone' => __( 'Organizer Phone', 'event-calendar-attendees' ),
            'event_cost' => __( 'Event Cost', 'event-calendar-attendees' ),
	        'event_website' => __( 'Event/Ticket Website', 'event-calendar-attendees' ),
            'excerpt' => __( 'Excerpt', 'event-calendar-attendees' ),
            'event_image' => __( 'Event Image', 'event-calendar-attendees' ),
	        'event_image_url' => __( 'Event Image (URL only)', 'event-calendar-attendees' ),
            'all_day' => __( 'All Day', 'event-calendar-attendees' ),
            'link' => __( 'Event Link', 'event-calendar-attendees' ),
            'link_url' => __( 'Event Link (URL only)', 'event-calendar-attendees' ),
	        'ical_link_url' => __( 'iCal Link (URL only)', 'event-calendar-attendees' ),
	        'gcal_link_url' => __( 'Google Calendar Link (URL only)', 'event-calendar-attendees' ),
            'recurring' => __( 'Recurring Description (if recurring)', 'event-calendar-attendees' ),
	        'categories' => __( 'Categories', 'event-calendar-attendees' ),
	        'category_links' => __( 'Category Links', 'event-calendar-attendees' ),
	        'tags' => __( 'Tags', 'event-calendar-attendees' ),
	        'tag_links' => __( 'Tag Links', 'event-calendar-attendees' ),
	        'colour' => __( 'Colour', 'event-calendar-attendees' ),
	        'featured' => __( 'Featured', 'event-calendar-attendees' ),
        );

        try {
            $retval = array();
            $feed = ECACalendarFeedFactory::create( $plugin_slug );
        ...
        return apply_filters( 'eca_available_format_tags_display', $retval, $plugin_slug );
    }
    ...
    private function sanitize_date( $date ) {
        ...
        elseif ( strtotime( $date ) !== FALSE )
            ...
            throw new Exception( __( 'Invalid date', 'event-calendar-attendees' ) );
    }
    ...
    public function get_excerpt() {
	    if ( ! $this->_excerpt ) {
		    ...
		    return apply_filters( 'eca_get_excerpt', wp_trim_words( $excerpt, 55, apply_filters( 'eca_excerpt_more', ' [&hellip;]', $this ) ), $this );
	    }
        return apply_filters( 'eca_get_excerpt', $this->_excerpt, $this );
    }
	...
	function get_category_link( $category ) {
		return '<a href="' . esc_url( get_term_link( $category ) ) . '" alt="' . esc_attr( sprintf( __( 'View all events in %s', 'event-calendar-attendees' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>';
	}

	function handle_format_tags( $output, $options = array() ) {
		foreach ( apply_filters( 'eca_available_format_tags', self::get_available_format_tags( $this->get_plugin() ) ) as $tag => $description ) {
			switch ( $tag ) {
				case 'tags':
				...
				case 'category_links':
					...
					break;
				case 'start_date':
					$output = str_replace( '{start_date}', apply_filters( 'eca_start_date_output', date_i18n( get_option( 'date_format' ), $this->get_start_date() ), $this, $options ), $output );
					break;
				case 'start_time':
					$output = str_replace( '{start_time}', apply_filters( 'eca_start_time_output', date_i18n( get_option( 'time_format' ), $this->get_start_date() ), $this, $options ), $output );
					break;
				case 'end_date':
					if ( $this->get_instant_event() )
						$output = str_replace( '{end_date}', '', $output );
					else
						$output = str_replace( '{end_date}', apply_filters( 'eca_end_date_output', date_i18n( get_option( 'date_format' ), $this->get_end_date() ), $this, $options ), $output );
					break;
				case 'end_time':
					if ( $this->get_instant_event() )
				...
				case 'all_day':
					if ( $this->get_all_day() )
						$output = str_replace( '{all_day}', __( 'All day', 'event-calendar-attendees' ), $output );
					else
				...
				case 'recurring':
					...
					elseif ( $this->get_repeat_frequency() > 0 and $this->get_repeat_interval() )
						$output = str_replace( '{recurring}', sprintf( _n( 'Occurs every %s %s', 'Occurs every %s %s', $this->get_repeat_frequency(), 'event-calendar-attendees' ), $this->get_repeat_frequency(), $this->get_repeat_interval_text( $this->get_repeat_frequency() ) ), $output );
					else
						$output = str_replace( '{recurring}', '', $output );
					break;
				case 'link':
					if ( $this->get_link() )
						$output = str_replace( '{link}', '<a href="' . $this->get_link() . '">'  . apply_filters( 'eca_event_link_text', __( 'More information', 'event-calendar-attendees' ) ) . '</a>', $output );
					else
				...
				default:
					...
					} elseif ( apply_filters( 'eca_handle_tag_output', false, $tag, $this ) )
						$output = str_replace( '{' . $tag . '}', apply_filters( 'eca_generate_custom_output', '', $tag, $this ), $output );
			}
		}
		return $output;		
	}
