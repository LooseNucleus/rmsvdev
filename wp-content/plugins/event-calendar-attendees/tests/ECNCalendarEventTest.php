<?php
class ECACalendarEventTest extends WP_UnitTestCase {
	...
	function testCategories() {
		...
		$this->assertEquals( '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all events in %s', 'event-calendar-attendees' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>', $event->get_from_format( '{category_links}' ), 'should output links for category');
